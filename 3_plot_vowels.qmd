...
Now to actually make the vowel plots! This document goes into detail about how I decided to make them the way I did and how to implement them in ggplot, but if you just want to see the final results, jump down to [here](#fig-gretchen-ls), [here](#fig-lauren-ls), and [here](#fig-paired-ep).
...
### Plot Vowel Boundaries

One of the the most common types of vowel plots draws ellipses around the vowel boundaries.

```{r}
#| label: fig-ellipses
#| fig-cap: "Vowel means (labels) and boundaries (ellipses)."

ellipses <- formants %>%
  ggplot(aes(x = F2, y = F1, group = Vowel)) +  # <1>
  geom_mark_ellipse(  # <2>
    aes(color = Vowel),  # <3>
    expand = 0  # <4>
  ) +
  geom_textbox(  # <5>
    data = . %>%  # <6>
      summarise(.by = c(Speaker, List, Vowel), F1 = mean(F1), F2 = mean(F2)),  # <6>
    aes(label = Vowel, fill = Vowel), # <7>
    color = lingthusiasm_navy, size = 4.5, halign = 0.5, valign = 0.5,  # <8>
    alpha = 0.8, box.color = NA,  # <9>
    width = unit(0.10, "snpc"), height = unit(0.10, "snpc"),  # <10>
    box.padding = unit(c(0, 0, 0, 0), "snpc"), box.r = unit(0.01, "snpc")  # <10>
  ) +
  facet_grid(Speaker ~ List) +
  scale_color_manual(values = carto_pal(12, "Bold")) +
...
For more info on ellipse vowel plots and how to tweak their appearance, I like [this tutorial](https://lingmethodshub.github.io/content/R/vowel-plots-tutorial/).

Save the plots created so far: 

```{r}
#| label: export-plots-1
...
#### Word Means for Wells Lexical Set {.unnumbered}
...
gretchen_words_ls <- formants %>%  # <1>
  filter(Speaker == "Gretchen" & List == "Wells Lexical Set") %>%  # <1>
  summarise(.by = Word, F1 = mean(F1), F2 = mean(F2)) %>%  # <2>
  ggplot(aes(x = F2, y = F1, label = Word)) +  # <3>
  geom_point(color = lingthusiasm_navy, size = 2) +  # <4>
  geom_label_repel(  # <5>
    color = lingthusiasm_navy,  # <6>
    family = "sans_alt", size = 5,  # <7>
    min.segment.length = 0, segment.size = 0.75, label.size = 1,  # <8>
    label.padding = unit(0.4, "lines"), label.r = unit(0.4, "lines"),  # <9>
    force = 65, seed = 84  # <10>
  ) +
  geom_label_repel(  # <11>
    color = lingthusiasm_green,  # <12>
    family = "sans_alt", size = 5,  # <13>
    min.segment.length = 0, segment.size = NA, label.size = NA,  # <14>
    label.padding = unit(0.4, "lines"), label.r = unit(0.4, "lines"),  # <15>
    force = 65, seed = 84  # <15>
  ) +
  scale_x_reverse(limits = c(3100, 500), expand = c(0, 0)) +  # <16>
  scale_y_reverse(limits = c(1200, 325), expand = c(0, 0)) +  # <16>
  theme_classic() +
  theme(  # <17>
    axis.line = element_blank(), axis.text = element_blank(),  # <17>
    axis.ticks = element_blank(), axis.title = element_blank()  # <17>
  ) +  # <17>
  annotate(  # <18>
    geom = "text", label = "Gretchen",  # <18>
    family = "sans_alt", size = 11, color = lingthusiasm_green,  # <19>
    x = 2525, y = 1070  # <20>
  ) +
  inset_element(  # <21>
    p = lingthusiasm_logo,  # <21>
    left = unit(0.05, "snpc"), right = unit(0.15, "snpc"),  # <22>
    top = unit(0.2, "snpc"), bottom = unit(0.1, "snpc")  # <22>
  )

ggsave(  # <23>
  gretchen_words_ls, path = "plots", filename = "gretchen_words_ls.png",  # <23>
  width = 8, height = 5, unit = "in", device = png  # <23>
)  # <23>
```
...
![Gretchen: Means for each word in the Wells Lexical Set data.](plots/gretchen_words_ls.png){#fig-gretchen-ls}
...
lauren_words_ls <- formants %>%
  filter(Speaker == "Lauren" & List == "Wells Lexical Set") %>%  # <1>
  ...
  scale_x_reverse(limits = c(3000, 500), expand = c(0, 0)) +  # <2>
  scale_y_reverse(limits = c(1000, 325), expand = c(0, 0)) +  # <2>
  theme_classic() +
  theme(
  ...
  inset_element(
    p = lingthusiasm_logo,
    left = unit(0.05, "snpc"), right = unit(0.15, "snpc"),  # <4>
    top = unit(0.25, "snpc"), bottom = unit(0.15, "snpc")  # <4>
  )
...
![Lauren: Means for each word in the Wells Lexical Set data.](plots/lauren_words_ls.png){#fig-lauren-ls}

#### Vowel Means for Lingthusiasm Episode Words {.unnumbered}
...
gretchen_vowels_ep <- formants %>%  # <1>
  filter(Speaker == "Gretchen" & List == "Lingthusiasm Episodes") %>%  # <1>
  summarise(.by = Vowel, F1 = mean(F1), F2 = mean(F2)) %>%  # <1>
  ggplot() +  # <2>
  geom_path(  # <3>
    data = vowel_polygon,  # <3>
    aes(x = x, y = y),  # <3>
    color = lingthusiasm_navy, linewidth = 2, lineend = "round"  # <4>
  ) +
  geom_mark_circle(  # <5>
    aes(x = F2, y = F1, group = Vowel),  # <5>
    radius = unit(0.048, "snpc"), n = 1000,  # <6>
    fill = lingthusiasm_green, alpha = 1, color = NA  # <7>
  ) +
  geom_text(  # <8>
    aes(x = F2, y = F1, label = Vowel),  # <8>
    color = "white", size = 11, nudge_y = 4  # <9>
  ) +
  scale_x_reverse(
    limits = c(max(vowel_polygon$x), min(vowel_polygon$x)),  # <10>
    expand = c(0.01, 0.01)  # <10>
  ) +
  scale_y_reverse(
    limits = c(max(vowel_polygon$y), min(vowel_polygon$y)),  # <10>
    expand = c(0.01, 0.01),  # <10>
    position = "right"  # <11>
  ) +
  theme_classic() +
  theme(
    axis.line = element_blank(),  # <12>
    axis.text = element_blank(),  # <12>
    axis.ticks = element_blank(),  # <12>
    axis.title.x = element_blank(),  # <12>
    axis.title.y = element_text(  # <13>
      size = 30, hjust = 1,  # <13>
      color = lingthusiasm_navy, family = "sans_alt"  # <13>
    )  # <13>
  ) +
  labs(y = "Gretchen")  # <14>
...
![Gretchen: Means for each vowel in the Lingthusiasm episode data.](plots/gretchen_vowels_ep.png){#fig-gretchen-ep}
...
lauren_vowels_ep <- formants %>%
  filter(Speaker == "Lauren" & List == "Lingthusiasm Episodes") %>%  # <1>
...
![Lauren: Means for each vowel in the Lingthusiasm episode data.](plots/lauren_vowels_ep.png){#fig-lauren-ep}
...
paired_vowels_ep <- gretchen_vowels_ep +  # <1>
  scale_x_continuous(  # <2>
    limits = c(min(vowel_polygon$x), max(vowel_polygon$x)),  # <2>
    expand = c(0.01, 0.01)
  ) +
  scale_y_reverse(
    limits = c(max(vowel_polygon$y), min(vowel_polygon$y)),
  ...
  theme(
    ...
    plot.margin = margin(t = 10, l = 10, r = 30, b = 10)  # <5>
  ) +
  lauren_vowels_ep +  # <6>
  theme(plot.margin = margin(t = 10, l = 30, r = 10, b = 10)) +  # <7>
  lingthusiasm_tagline +  # <8>
  plot_layout(design = c(  # <9>
    area(t = 1, l = 1, b = 5, r = 4),  # <10>
    area(t = 1, l = 5, b = 5, r = 8),  # <11>
    area(t = 4, l = 4, b = 5, r = 5)   # <12>
  ))
...
11. Lauren's plot: full height, right side.
12  Lingthusiasm tagline logo: 20% of height, overlapping each of the other plots by 25%.

![Gretchen (left) & Lauren (right): Means for each vowel in the Lingthusiasm episode data.](plots/paired_vowels_ep.png){#fig-paired-ep}
